 ...
 * @since 0.1.0
 */
public class OptimumAverageAllocator implements AllocatorInterface {
    ...
    public List<Allocation> allocate(List<Demand> demands) {
        ...
        List<Demand> pending = new ArrayList<Demand>();
        double allocated = 0, average = getDemandAverage(demands);
        List<ProvisionTime> slots = getSlots(demands);
        HashMap<ProvisionTime, Double> allocatedMap = new HashMap<ProvisionTime, Double>();


        for (Demand d : demands) {
            ConsumptionDemand cd = (ConsumptionDemand) d;
            if (!allocatedMap.containsKey(cd.getProvisionTime())) {
                allocatedMap.put(cd.getProvisionTime(), 0.0);
            }
            allocated = allocatedMap.get(cd.getProvisionTime());
            if (allocated < average) {
                list.add(new ConsumptionAllocation(cd.getAgent(), cd.getProvisionTime(), cd.
                        getAmount()));
                allocatedMap.put(cd.getProvisionTime(), allocated + cd.getAmount());
            } else {
                pending.add(d);
            }
        }
        ...
        while (!pending.isEmpty()) {
            Demand d = getRandomDemandAndRemove(pending);
            boolean done = false;

            while (!done) {
                for (ProvisionTime pt : allocatedMap.keySet()) {
                    allocated = allocatedMap.get(pt);
                    if (allocated < average) {
                        ConsumptionDemand cd = (ConsumptionDemand)d;
                        list.add(new ConsumptionAllocation(cd.getAgent(), pt, cd.
                                getAmount()));
                        allocatedMap.put(pt, allocated + cd.getAmount());
